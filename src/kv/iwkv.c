...
#define IS_IWKV_LIGHT_ERROR(rc_) \
  ((rc_) == IWKV_ERROR_NOTFOUND)
...
static void _lx_release(IWLCTX *lx) {
  ...
  if (lx->nlvl > -1) {
    for (int i = SLEVELS - 1; i >= 0; --i) {
      if (lx->pupper[i] && lx->pupper[i]->addr != laddr) {
        _sblk_release(&lx->pupper[i]);
        laddr = lx->pupper[i]->addr;
        lx->pupper[i] = 0;
      }
      if (lx->plower[i] && lx->plower[i]->addr != laddr) {
        _sblk_release(&lx->plower[i]);
        laddr = lx->plower[i]->addr;
        lx->plower[i] = 0;
      }
    }
  } else {
...
static iwrc _lx_split_addkv(IWLCTX *lx, SBLK **onb) {
  iwrc rc;
  int idx;
  uint8_t *mm, kvbpow = 0;
  SBLK *nb, *lb = lx->lower;
...
IW_INLINE iwrc _lx_get_lr(IWLCTX *lx) {
  ...
  if (lx->lower) {
    ...
    rc = fsm->acquire_mmap(fsm, 0, &mm, 0);
    RCRET(rc);
    int idx = _sblk_find_pi(lx->lower, lx->key, mm);
    if (idx >= 0 || idx < lx->lower->pnum) {
      idx = lx->lower->pi[idx];
