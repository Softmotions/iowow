...
IW_INLINE int _cmp_key2(iwdb_flags_t dbflg, const void *v1, int v1len, const IWKV_val *key) {
  if (dbflg & IWDB_COMPOUND_KEYS) {
    // Compound keys mode
    int ret;
    const char *u1 = v1, *u2 = key->data;
    ...
    if (dbflg & IWDB_VNUM64_KEYS) {
      ...
      ret = strncmp(u2, u1, MIN(v1len, v2len));
    }
    if (ret == 0) {
...
IW_INLINE int _cmp_key(iwdb_flags_t dbflg, const void *v1, int v1len, const IWKV_val *key) {
  int rv = _cmp_key2(dbflg, v1, v1len, key);
...
static WUR iwrc _sblk_create(IWLCTX *lx, uint8_t nlevel, uint8_t kvbpow, off_t baddr, SBLK **oblk) {
  iwrc rc;
  SBLK *sblk;
  KVBLK *kvblk;
  off_t blen;
...
static WUR iwrc _sblk_find_pi_mm(SBLK *sblk, IWLCTX *lx, const uint8_t *mm, bool *found, uint8_t *idxp) {
  *found = false;
  ...
  while (1) {
    ...
    RCRET(rc);
    int cr = _cmp_key(dbflg, k, kl, lx->key);
    if (!cr) {
      *found = true;
      break;
    } else if (cr < 0) {
  ...
  if (!rc) {
    ...
    for (IWKV_cursor cur = db->cursors; cur; cur = cur->next) {
      if (cur->cn && cur->cn->addr == sblk->addr) {
        ...
        if (cur->cnpos > 0 && (cur->cnpos > idx || cur->cnpos == sblk->pnum)) {
          cur->cnpos--;
        }
      }
    }
...
WUR iwrc _lx_sblk_cmp_key(IWLCTX *lx, SBLK *sblk, int *resp) {
  ...
  if ((sblk->flags & SBLK_FULL_LKEY)
    ...
    res = _cmp_key(dbflg, sblk->lk, lkl, key);
  } else {
    res = _cmp_key2(dbflg, sblk->lk, lkl, key);
    if (!res) {
      uint32_t kl;
      uint8_t *mm, *k;
      IWFS_FSM *fsm = &lx->db->iwkv->fsm;
...
static iwrc _lx_split_addkv(IWLCTX *lx, int idx, SBLK *sblk) {
  ...
  if (uside) { // Upper side
    ...
    rc = _sblk_create(lx, (uint8_t) lx->nlvl, kvbpow, sblk->addr, &nb);
    RCRET(rc);
...
static WUR iwrc _dbcache_cmp_nodes(const void *v1, const void *v2, void *op, int *res) {
  ...
  *res = _cmp_key2(dbflg, k1, kl1, &key2);
  if (*res == 0 && (!cn1->fullkey || !cn2->fullkey)) {
    ...
    *res = _cmp_key(dbflg, k1, kl1, &key2);
  }
...
finish:
  ...
  if (rc && rc != IWKV_ERROR_NOTFOUND) {
    if (cur->cn) _sblk_release(lx, &cur->cn);
  }
  return rc;
}
...
iwrc iwkv_new_db(IWKV iwkv, iwdb_flags_t dbflg, uint32_t *dbidp, IWDB *dbp) {
  ...
  *dbidp = 0;
  ...
  for (khiter_t k = kh_begin(iwkv->dbs); k != kh_end(iwkv->dbs); ++k) {
    if (!kh_exist(iwkv->dbs, k)) continue;
...
finish2:
    ...
    RCGO(rc, finish);
    if (cur->cnpos == sblk->pnum - 1) {
      // Last pair in block deleted
      cur->skip_next = -1;
    } else {
      cur->skip_next = 1;
    }
