...
static iwrc _iwkv_get_wl_set_open_false(IWKV iwkv) {
  int rci = 0;
  API_WLOCK(iwkv, rci);
  iwkv->open = false;
  return 0;
    ...
    for (i = 0; i < KVBLK_IDXNUM; ++i) {
      if (tidx[i].off == koff) {
        if (koff - (i > 0 ? tidx[i - 1].off : 0) >= rsize) {
          ...
          RCGO(rc, finish);
          rc = _kvblk_addkv(kb, key, uval, idxp, op_flags, true);
        }
        break;
      }
    }
...
IW_INLINE iwrc _sblk_addkv2(IWLCTX *lx,
  ...
  iwrc rc = _kvblk_addkv(kvblk, key, val, &kvidx, lx->op_flags, false);
  RCRET(rc);
  if (sblk->pnum - idx > 0) {
...
IW_INLINE iwrc _sblk_addkv(SBLK *sblk,
  ...
  if (!internal && (op_flags & IWKV_DUP_REMOVE)) {
    return IWKV_ERROR_NOTFOUND;
  }
  iwrc rc = _kvblk_addkv(kvblk, key, val, &kvidx, op_flags, internal);
  RCRET(rc);
  rc = fsm->acquire_mmap(fsm, 0, &mm, 0);
  RCRET(rc);
...
IW_INLINE iwrc _sblk_create2(IWLCTX *lx,
  ...
  *oblk = 0;
  ...
  RCRET(rc);
  rc = _sblk_addkv(sblk, key, val, lx->op_flags, false);
  if (rc) {
...
static iwrc _lx_split_addkv(IWLCTX *lx, int idx, SBLK *sblk) {
  ...
  if (idx == sblk->pnum) {
    ...
    for (int i = pivot, end = sblk->pnum; i < end; ++i) {
      ...
      fsm->release_mmap(fsm);
      RCBREAK(rc);
      rc = _sblk_addkv(nb, &key, &val, lx->op_flags, true);
      _kv_dispose(&key, &val);
      RCBREAK(rc);
    ...
    rc = _sblk_addkv(idx > pivot ? nb : sblk, lx->key, lx->val, lx->op_flags, false);
    RCGO(rc, finish);
  }
  // fix levels
  //  [ lb -> sblk -> ub ]
...
IW_INLINE WUR iwrc _lx_addkv(IWLCTX *lx) {
  ...
  if (!found && sblk->pnum >= KVBLK_IDXNUM) {
    if (lx->nlvl < 0 && idx == sblk->pnum && lx->upper && lx->upper->pnum < KVBLK_IDXNUM) {
      ...
      mm = 0;
      // Good to place lv into the right(upper) block
      rc = _sblk_addkv(lx->upper, lx->key, lx->val, lx->op_flags, false);
      goto finish;
    }
    if (lx->nlvl < 0) {
  ...
  if (!found && sblk->pnum >= KVBLK_IDXNUM) {
    ...
    if (!found) {
      ...
      rc = _sblk_updatekv(sblk, idx, lx->key, lx->val, lx->op_flags);
    }
  }
finish:
  if (mm) {
...
IW_INLINE iwrc _lx_get_le_idx_lr(IWLCTX *lx, int8_t *oidx) {
  iwrc rc = _lx_find_bounds(lx, false);
  RCRET(rc);
  bool found;
  uint8_t *mm;
  ...
  if (found) {
    *oidx = idx;
    ...
    if ((lx->lower->flags & SBLK_DB) || lx->lower->pnum < 1) {
      rc = IWKV_ERROR_NOTFOUND;
    } else {
      *oidx = idx;
    }
...
static iwrc _cursor_to_lr(IWKV_cursor *cur, IWKV_cursor_op op) {
  IWDB db = cur->lx.db;
  IWFS_FSM *fsm = &db->iwkv->fsm;
  IWLCTX *lx = &cur->lx;
  uint8_t *mm = 0;
  iwrc rc = fsm->acquire_mmap(fsm, 0, &mm, 0);
  RCGO(rc, finish);
  if (op < IWKV_NEXT) { // IWKV_FIRST | IWKV_LAST
    if (cur->cn) {
...
start:
  ...
  } else { // IWKV_KEY_EQ | IWKV_KEY_GE
    if (!lx->key) {
      rc = IW_ERROR_INVALID_STATE;
      goto finish;
    }
    rc = _lx_get_le_idx_lr(lx, &cur->cnpos);
    if (lx->upper) {
      _sblk_release(lx, &lx->upper);
    }
    cur->cn = lx->lower;
...
iwrc iwkv_close(IWKV *iwkvp) {
  ...
  int rci;
  IWKV iwkv = *iwkvp;
  iwrc rc = _iwkv_wait_no_workers_then_call(iwkv, _iwkv_get_wl_set_open_false);
...
iwrc iwkv_cursor_open(IWDB db, IWKV_cursor *cur, IWKV_cursor_op op, const IWKV_val *key) {
  if (!db || !db->iwkv || !cur) {
    return IW_ERROR_INVALID_ARGS;
  }
  if ((key && op < IWKV_KEY_EQ) || op < IWKV_FIRST) {
    return IW_ERROR_INVALID_ARGS;
  }
  iwrc rc;
  int rci;
...
iwrc iwkv_cursor_close(IWKV_cursor *cur) {
  iwrc rc = 0;
  int rci;
  if (!cur) {
    return IW_ERROR_INVALID_ARGS;
  ...
  if (cur->cn) {
    if ((cur->cn->flags & SBLK_DURTY) ||
        (cur->cn->kvblk && (cur->cn->kvblk->flags & KVBLK_DURTY))) {
...
finish:
  IWRC(_db_worker_dec(cur->lx.db), rc);
...
iwrc iwkv_cursor_to_key(IWKV_cursor *cur, IWKV_cursor_op op, const IWKV_val *key) {
  int rci;
  if (!cur || (op != IWKV_KEY_EQ && op != IWKV_KEY_LE)) {
    return IW_ERROR_INVALID_ARGS;
  }
  if (!cur->lx.db) {
    return IW_ERROR_INVALID_STATE;
...
iwrc iwkv_cursor_get(IWKV_cursor *cur, IWKV_val *okey, IWKV_val *oval) {
  int rci;
  iwrc rc = 0;
  if (!cur) {
    return IW_ERROR_INVALID_ARGS;
