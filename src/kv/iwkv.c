...
IW_INLINE void _aln_release(IWDB db, blkn_t blkn) {
...
IW_INLINE iwrc _aln_acquire(IWDB db, blkn_t blkn) {
  ALN *aln;
  int rci;
  iwrc rc = 0;
...
finish:
  pthread_spin_unlock(&db->sl);
  if (!rc) {
    rci = pthread_mutex_lock(&aln->mtx);
    if (rci) {
...
IW_INLINE void _sblk_release(IWLCTX *lx, SBLK **sblkp) {
  ...
  if (sblk->flags & SBLK_WLOCKED) {
    _aln_release(lx->db, ADDR2BLK(sblk->addr));
  }
  sblk->flags = 0;
  sblk->kvblk = 0;
  *sblkp = 0;
...
static iwrc _sblk_create(IWLCTX *lx,
  ...
  *oblk = 0;
  ...
  sblk->flags |= SBLK_WLPASS;
  *oblk = sblk;
  AAPOS_INC(lx->saan);
  return rc;
}

static iwrc _sblk_at(IWLCTX *lx, off_t addr, sblk_flags_t flgs, bool keepwl, SBLK **sblkp) {
  iwrc rc;
  uint8_t *mm;
  uint32_t lv;
  *sblkp = 0;
...
IW_INLINE void _sblk_sync_mm(SBLK *sblk, uint8_t *mm) {
  if (sblk->flags & SBLK_DURTY) {
    assert(sblk->flags & (SBLK_WLOCKED | SBLK_WLPASS));
    sblk->flags &= ~SBLK_DURTY;
    uint32_t lv;
...
static int _sblk_find_pi_mm(SBLK *sblk,
                            const IWKV_val *key,
                            const uint8_t *mm,
                            bool *found) {
  if (sblk->flags & SBLK_DB) {
...
static int _sblk_insert_pi_mm(SBLK *sblk,
                              int8_t nidx,
                              const IWKV_val *key,
                              const uint8_t *mm) {
  assert(sblk->kvblk);
...
IW_INLINE iwrc _sblk_updatekv(SBLK *sblk,
                              ...
                              const IWKV_val *val,
                              iwkv_opflags opflags) {
  assert(sblk && sblk->kvblk && idx >= 0 && idx < sblk->pnum && (sblk->flags & SBLK_WLOCKED));
  KVBLK *kvblk = sblk->kvblk;
...
IW_INLINE iwrc _lx_sblk_cmp_key(IWLCTX *lx, SBLK *sblk, int *res) {
  ...
  if (IW_UNLIKELY(sblk->pnum < 1 || (sblk->flags & SBLK_DB))) { // empty block
    ...
    *res = _cmp_key(dbflg, sblk->lk, sblk->lkl, key->data, key->size);
    ...
    _kvblk_peek_key(sblk->kvblk, sblk->pi[0], mm, &k, &kl);
    if (!kl) {
      *res = 0;
      return IWKV_ERROR_CORRUPTED;
    }
  ...
  while ((blkn = lx->lower->n[lvl])) {
    ...
    if (lx->nlvl > -1 && lvl < lx->nlvl) {
      ...
      if (lx->pupper[ulvl] && lx->pupper[ulvl]->addr == blkaddr) {
        ...
        rc = _sblk_at(lx, blkaddr, 0, false, &sblk);
      }
    } else {
...
static iwrc _lx_find_bounds(IWLCTX *lx) {
  assert(!lx->lower);
  iwrc rc = _sblk_at(lx, lx->db->addr, 0, false, &lx->lower);
  RCRET(rc);
  lx->dblk = lx->lower;
start:
  if (lx->nlvl > lx->dblk->lvl) { // New level in DB
    rc = _sblk_write_upgrade(lx, lx->dblk);
...
IW_INLINE iwrc _lx_lock_chute(IWLCTX *lx) {
  assert(lx->nlvl >= 0);
  iwrc rc = 0;
  if (!lx->pupper[lx->nlvl]) { // fix zero upper by dbtail
    SBLK *dbtail;
    rc = _sblk_at(lx, 0, 0, false, &dbtail);
    RCRET(rc);
  ...
  for (int i = lx->nlvl; i >= 0; --i) {
    rc = _sblk_write_upgrade(lx, lx->plower[i]);
    RCRET(rc);
    rc = _sblk_write_upgrade(lx, lx->pupper[i]);
    RCRET(rc);
    if (lx->plower[i]->n[i] != ADDR2BLK(lx->pupper[i]->addr)) {
...
IW_INLINE WUR iwrc _lx_addkv(IWLCTX *lx) {
  ...
  if (lx->nlvl < 0) {
    rc = _sblk_write_upgrade(lx, sblk);
    RCRET(rc);
  } else {
    rc = _lx_lock_chute(lx);
    RCRET(rc);
  }
  ...
  if (!found && sblk->pnum >= KVBLK_IDXNUM) {
    if (lx->nlvl < 0 && idx == sblk->pnum && lx->upper && lx->upper->pnum < KVBLK_IDXNUM) {
      rc = _sblk_write_upgrade(lx, lx->upper);
      RCRET(rc);
  ...
  if (!found && sblk->pnum >= KVBLK_IDXNUM) {
    ...
    if (!found) {
      return _sblk_addkv2(lx, sblk, idx, lx->key, lx->val);
    } else {
...
IW_INLINE iwrc _lx_get_lr(IWLCTX *lx) {
  ...
  RCGO(rc, finish);
  int idx = _sblk_find_pi_mm(lx->lower, lx->key, mm, &found);
  if (found) {
    idx = lx->lower->pi[idx];
...
IW_INLINE iwrc _lx_del_lr(IWLCTX *lx, bool dbwlocked) {
  ...
  rc = _lx_find_bounds(lx);
  RCRET(rc);
  sblk = dbwlocked ? lx->upper : lx->lower;
  assert(sblk);
  if (!dbwlocked) {
    rc = _sblk_write_upgrade(lx, sblk);
    RCGO(rc, finish);
  ...
  if (sblk->pnum <= 1) {
    if (!dbwlocked) {
      ...
      goto finish;
    } else if (lx->nlvl != sblk->lvl || sblk->addr != lx->upper_addr) {
      int8_t nlvl = sblk->lvl;
      _lx_release_mm(lx, 0);
      lx->nlvl = nlvl;
      lx->upper_addr = sblk->addr;
  ...
  if (lx->upper->pnum < 1) {
    ...
    SBLK *nb;
    assert(!lx->nb);
    rc = _sblk_at(lx, BLK2ADDR(lx->upper->n[0]), 0, false,  &nb);
    RCGO(rc, finish);
    lx->nb = nb;
    lx->nb->p0 = lx->upper->p0;
    lx->nb->flags |= SBLK_DURTY;
    lx->nb->flags |= SBLK_WLPASS;
...
IW_INLINE iwrc _cursor_get_ge_idx(IWLCTX *lx, IWKV_cursor_op op, int8_t *oidx) {
  iwrc rc = _lx_find_bounds(lx);
  RCRET(rc);
  bool found;
  uint8_t *mm;
...
start:
  if (op < IWKV_CURSOR_EQ) { // IWKV_CURSOR_NEXT | IWKV_CURSOR_PREV
    ...
    if (!cur->cn) {
      if (cur->dbaddr) {
        rc = _sblk_at(lx, (cur->dbaddr < 0 ? 0 : cur->dbaddr), 0, cur->update, &cur->cn);
        cur->dbaddr = 0;
        RCGO(rc, finish);
      } else {
    ...
    if (op == IWKV_CURSOR_NEXT) {
      if (cur->cnpos + 1 >= cur->cn->pnum) {
        ...
        rc = _sblk_at(lx, BLK2ADDR(n), 0, cur->update, &cur->cn);
        RCGO(rc, finish);
        cur->cnpos = 0;
    ...
    if (cur->update) {
      rc = _sblk_write_upgrade(lx, lx->lower);
      if (rc == _IWKV_ERROR_AGAIN) {
        rc = 0;
        _lx_release_mm(lx, 0);
        goto start;
...
iwrc iwkv_cursor_set(IWKV_cursor cur, IWKV_val *val, iwkv_opflags opflags) {
  ...
  if (!cur->cn || !cur->lx.db || (cur->cn->flags & SBLK_DB) || !(cur->cn->flags & SBLK_WLOCKED)) {
    return IW_ERROR_INVALID_STATE;
  }
  IWDB db = cur->lx.db;
  API_DB_RLOCK(db, rci);
  ...
  for (int i = 0, j = 0; i < sb->pnum; ++i, ++j) {
    ...
    if (flags & IWKVD_PRINT_VALS) {
      ...
      if (sb->db->dbflg & IWDB_UINT64_KEYS) {
        ...
        memcpy(&k, kbuf, sizeof(k));
        k = IW_ITOHLL(k);
        fprintf(f, "    [%02d,%02d] %lu", i, sb->pi[i], k);
      } else if (sb->db->dbflg & IWDB_UINT32_KEYS) {
        uint32_t k;
        memcpy(&k, kbuf, sizeof(k));
        k = IW_ITOHL(k);
        fprintf(f, "    [%02d,%02d] %u", i, sb->pi[i], k);
      } else {
...
void iwkvd_db(FILE *f, IWDB db, int flags) {
  ...
  lx.sblk_flags |= SBLK_NO_LOCK;
  iwrc rc = _sblk_at(&lx, db->addr, 0, false, &sb);
  if (rc) {
    iwlog_ecode_error3(rc);
    return;
  }
  rc = _sblk_at(&lx, 0, 0, false, &tail);
  if (rc) {
    iwlog_ecode_error3(rc);
    return;
  }
  ...
  blkn_t blk = sb->n[0];
  while (blk) {
    rc = _sblk_at(&lx, BLK2ADDR(blk), 0, false, &sb);
    if (rc) {
      iwlog_ecode_error3(rc);
      return;
    }
