...
IW_INLINE iwrc _lx_sblk_cmp_key(IWLCTX *lx, SBLK *sblk, int *res) {
  ...
  if (IW_UNLIKELY(sblk->pnum < 1 || (sblk->flags & SBLK_DB))) { // empty block
    *res = 0;
    return IWKV_ERROR_CORRUPTED;
  } else if (sblk->flags & SBLK_FULL_LKEY) {
...
static iwrc _lx_find_bounds(IWLCTX *lx) {
  assert(!lx->lower);
  iwrc rc = _sblk_at(lx, lx->db->addr, 0, &lx->lower);
  RCRET(rc);
  if (lx->nlvl > lx->lower->lvl) { // todo review?
...
start:
  if (op < IWKV_CURSOR_EQ) { // IWKV_CURSOR_NEXT | IWKV_CURSOR_PREV
    ...
    if (!cur->cn) {
      if (cur->dbaddr) {
        rc = _sblk_at(lx, (cur->dbaddr < 0 ? 0 : cur->dbaddr), mm, 0, &cur->cn);
        cur->dbaddr = 0;
        RCGO(rc, finish);
      } else {
    ...
    if (op == IWKV_CURSOR_NEXT) {
      if (cur->cnpos + 1 >= cur->cn->pnum) {
        ...
        _sblk_sync_and_release_mm(lx, &cur->cn, mm);
        rc = _sblk_at_mm(lx, BLK2ADDR(n), mm, 0, &cur->cn);
        RCGO(rc, finish);
        cur->cnpos = 0;
...
void iwkvd_db(FILE *f, IWDB db, int flags) {
  ...
  lx.sblk_flags |= SBLK_NO_LOCK;
  rc = _sblk_at_mm(&lx, db->addr, mm, 0, &sb);
  if (rc) {
    iwlog_ecode_error3(rc);
    return;
  }
  rc = _sblk_at_mm(&lx, 0, mm, 0, &tail);
  if (rc) {
    iwlog_ecode_error3(rc);
    return;
  }
  ...
  blkn_t blk = sb->n[0];
  while (blk) {
    rc = _sblk_at_mm(&lx, BLK2ADDR(blk), mm, 0, &sb);
    if (rc) {
      iwlog_ecode_error3(rc);
      return;
    }
