...
#define KVBLK_MAX_NKV_SZ (KVBLK_HDRSZ + KVBLK_IDXNUM * (KVP_MAX_LEN_VLEN + KVP_MAX_OFF_VLEN))
...
#define SOFF_LK           (SOFF_LKL_U1 + 1)
#define SOFF_LVL_U1       (SOFF_LK + SBLK_LKLEN)
...
static iwrc _kvblk_at_mm(IWLCTX *lx,
  ...
  *blkp = 0;
  ...
  if (IW_UNLIKELY(kb->idxsz > (KVP_MAX_OFF_VLEN + KVP_MAX_LEN_VLEN) * KVBLK_IDXNUM)) {
    rc = IWKV_ERROR_CORRUPTED;
    goto finish;
  }
...
static void _sblk_sync_mm(SBLK *sblk, uint8_t *mm) {
  if (sblk->flags & SBLK_DURTY) {
    ...
    if (IW_UNLIKELY(sblk->flags & SBLK_DB)) {
      ...
      assert(wp - (mm + sblk->addr) <= SBLK_SZ);
    }
  }
...
IW_INLINE iwrc _sblk_addkv2(SBLK *sblk,
                            int8_t idx,
                            const IWKV_val *key,
                            const IWKV_val *val,
                            iwkv_opflags opflags,
