...
static void iwkv_test5(void) {
  ...
  rc = iwkv_cursor_open(db1, &cur1, IWKV_CURSOR_EQ, 0, &key);
  CU_ASSERT_EQUAL_FATAL(rc, 0);
...
static void iwkv_test1(void) {
  ...
  rc = iwkv_cursor_open(db1, &cur1, IWKV_CURSOR_BEFORE_FIRST, 0, 0);
  CU_ASSERT_EQUAL_FATAL(rc, 0);
  ...
  rc = iwkv_cursor_open(db1, &cur1, IWKV_CURSOR_AFTER_LAST, 0, 0);
  CU_ASSERT_EQUAL_FATAL(rc, 0);
  ...
  while (!(rc = iwkv_cursor_to(cur1, IWKV_CURSOR_PREV))) {
    ...
    //fprintf(stderr, "\n%.*s:%.*s", (int) key.size, (char *) key.data, (int) val.size, (char *) val.data);
    snprintf(kbuf, KBUFSZ, "%03dkkk", i);
    snprintf(vbuf, VBUFSZ, "%03dval", i);
  ...
  // Cursor set
  rc = iwkv_cursor_open(db1, &cur1, IWKV_CURSOR_EQ, IWKV_CURSOR_FOR_UPDATE, &key);
  CU_ASSERT_EQUAL_FATAL(rc, 0);
