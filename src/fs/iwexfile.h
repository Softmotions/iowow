 ...
 *    operations.
 * 
 * File operations implemented as function pointers contained in `IWFS_EXT` `C` structure.
 * The `iwrc iwfs_exfile_open(IWFS_EXT *f, const IWFS_EXT_OPTS *opts);` opens file and initializes a given `IWFS_EXT` structure. 
 */
...
typedef struct IWFS_EXT_OPTS {
    ...
    int                     use_locks;      /**< If `1` file operations will be guarded by rw lock. Dafault: `0` */
    IW_EXT_RSPOLICY         rspolicy;       /**< File resize policy function ptr. Default: `exact size policy`  */
...
typedef struct IWFS_EXT {
     ...
     * @param [out] mm Pointer assigned to start of mmaped region of `NULL` if error occurred .
 ...
 */
IW_EXPORT WUR iwrc iwfs_exfile_open(IWFS_EXT *f,
