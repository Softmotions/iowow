...
static iwrc lock(IWRLOCK *lk, off_t start, off_t len, iwrl_lockflags lflags,
                 uint64_t bsleepms, uint64_t asleepms,
                 int is_unlock) {
    iwrc rc = 0;
...
int main() {
    ...
    if (
        (NULL == CU_add_test(pSuite, "test_iwrlock1", test_iwrlock1)) ||
