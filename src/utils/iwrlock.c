...
#define _IWRL_IS_OWNER(IW_l_) \
    (pthread_equal((IW_l_)->owner, pthread_self()))

#define _IWRL_IS_OWNER2(IW_l_, IW_o_) \
    (pthread_equal((IW_l_)->owner, IW_o_))
...
static iwrc _iwrl_lock_append(IWRLOCK *lk, off_t start, off_t end,
                              int lflags, pthread_t owner) {
...
static iwrc _iwrl_range_lock(IWRLOCK *lk, off_t start, off_t end,
                             int lflags, pthread_t owner) {
    iwrc rc = 0;
    int remove_lr;
...
static iwrc _iwrl_range_wait(IWRLOCK *lk, off_t start, off_t end,
                             int lflags, pthread_t owner) {
    iwrc rc = 0;
